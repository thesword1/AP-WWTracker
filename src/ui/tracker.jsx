...
class Tracker extends React.PureComponent {
  ...
  async initialize() {
    ...
    const processQueueLocations = () => {
      ...
      if (data.includes(
        archipelagoConnection.getName() +' found their') ||
        data.includes(archipelagoConnection.getName() + ' sent')) {
          console.log(data);
          this.toggleArchipelagoLocationChecked(data);
      }
  ...
  toggleArchipelagoLocationChecked(params) {
    ...
    if (!trackerState.isLocationChecked(generalLocation, detailedLocation)){
      newTrackerState = trackerState.toggleLocationChecked(generalLocation,detailedLocation);
      this.setState({
        lastLocation: {
          generalLocation,
          detailedLocation,
